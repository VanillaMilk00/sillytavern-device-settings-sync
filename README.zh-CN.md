...
- **自动上传**：浏览器原生通知发现 localStorage 有变化后，只先记录键名。同一键连续修改会合并，停止变化 10 秒后整理一批，最多合并 15 秒，批次至少间隔 30 秒。只读取、计算和发送变更的键，不定时扫描整份数据。哈希及大型值差分在 Web Worker 运行。模型请求或其他同步操作进行中时会延后发送。
