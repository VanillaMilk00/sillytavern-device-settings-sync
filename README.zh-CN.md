...
在“设置”中开启 IndexedDB 手动同步并选好范围后，按钮会改为“上传／下载 localStorage＋已选 IndexedDB”，一并处理选中项目。IndexedDB 不会随自动上传或下载运行；配置仅保存在当前网站／账户／设备。
...
在来源设备按“上传 localStorage”，其他设备登录同一账户后按“下载 localStorage”。下载完成自动刷新。上传会同步移除那些远端存在、但来源设备已删除的可移植键。
