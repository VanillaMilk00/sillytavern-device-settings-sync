...
当前版本：**1.10.0**。IndexedDB 只会把明确选中的项目纳入手动同步，不会随 localStorage 自动同步。要使用 localStorage 增量自动保存，请将服务器插件更新至 1.10.0 并重启 SillyTavern；前端更新后刷新浏览器即可。升级后旧版自动上传会关闭，需重新确认才能启用新机制。

## 功能

- 默认纯手动，上传与下载前均有确认弹窗。两个自动开关均关闭时，加载页面不下载、不上传、不轮询、不备份。
- 自动上传和自动下载分别启用：上传由 localStorage 原生变更触发并只发送差异；下载在进入页面时检查。
...
在“设置”中开启 IndexedDB 手动同步并选好范围后，两个方向按钮会加上“＋已选 IndexedDB”，一并处理选中项目。IndexedDB 不会随自动上传或下载运行；配置仅保存在当前网站／账户／设备。
...
两个开关默认**关闭**，开启前会确认。偏好按网站、账户和浏览器设备保存，不随普通同步传到其他设备。升级后旧版自动上传会关闭，需重新确认才能启用新行为。

- **自动上传**：浏览器原生通知发现 localStorage 有变化后，只先记录键名。同一键连续修改会合并，停止变化 2 秒后整理一批，最多合并 15 秒，批次至少间隔 5 秒。只读取、计算和发送变更的键；不再等待模型请求空闲 15 分钟，也不定时扫描整份数据。哈希及大型值差分在 Web Worker 运行。
- **自动下载**：保持独立；新页面或手动刷新时检查，切换标签页不下载。只应用服务器单独变化，保留本机单独变化；成功套用后刷新一次。
...
隐藏的同源页面框接收浏览器原生 localStorage 变更通知，不会改写 `setItem` 等方法。已知生成 fetch／XHR 与原生生成事件只用于避免在模型请求期间传输；不读取提示词、响应、凭证或 URL 参数。跨源页面框、Worker 和未知传输无法完整观察，必要时自动上传会暂停。扩展可通过主页面接口报告请求生命周期，不传内容：
...
首次启用、重新打开页面或改变同步范围时会分段核对一次本机与服务器基准；空闲时不扫描整份数据，也不定时发网络请求。下载仍按所选普通／全部／勾选范围处理 localStorage，IndexedDB 用户数据保持手动同步。关闭自动上传会清除尚未送出的上传队列，不会关闭自动下载。

自动模式需要安全 HTTPS／localhost、Web Locks、Web Crypto、Web Worker、Resource Timing 及原生 `storage` 通知。请保持原浏览器登录；其他设备仍要手动同步或自行运行自动下载检查才能获取新数据。服务器及本机备份可能包含凭证，请妥善保护。

偏好、基准和排程是同步器内部数据：普通同步排除，完整本机救援备份保留，还原默认跳过。自动佇列使用独立内部 IndexedDB，不会出现在用户的 IndexedDB 管理或同步范围。

## 安装与升级

需要 SillyTavern 1.14.0+、Node.js 18+，并启用 Server Plugins。v1.10.0 的 119 项单元／行为测试和语法检查已通过；另在隔离的 SillyTavern 1.14.0、Node.js 22、Edge Chromium 环境通过 30 项管理界面和 14 项自动模式浏览器检查，涵盖桌面／手机尺寸及三语界面。测试也确认同一键快速连续变更会合并、自动下载不会在每次模型请求后重扫存储空间，且背景工作者不可用时不会阻塞前景保存。Firefox、WebKit 和实体手机尚未验证；旧版同步功能也曾在 SillyTavern 1.18.0 测试。这些结果不代表所有模型扩展、浏览器和实体手机均已验证。CI 覆盖 Node.js 18、20、24。
...
在来源设备按“上传本机设置（localStorage）”，其他设备登录同一账户后按“从服务器同步（localStorage）”。同步到本机后自动刷新。同步至服务器时，也会移除那些远端存在、但来源设备已删除的可移植键。
...
打开「管理 localStorage → localStorage 备份」才加载列表。每格显示时间、设备、原因、键数、估算容量和 JSON 大小，支持浏览、导出及恢复；选中时才读取内容。「导入、恢复或删除本机数据前也自动备份」默认**开启**。保留已保存的开关选择；旧版已关闭者，请在管理窗口手动开启。
...
- `GET /health`：能力标记新增 `incremental-localstorage-v1`。要启用 v1.10.0 自动上载，服务器插件也必须是 1.10.0 并重启；旧版会保留手动同步但暂停新自动上载。
...
- 「localStorage 清理 → 立即分析」扫描全部键，按容量排序并标记最大的十项。绿色为疑似缓存／临时／调试／日志，黄色需人工判断，红色受批量清理保护。仅按键名分类，不是 AI，也不能证明数据已经无用。
- 保护规则优先：凭证、历史、草稿、聊天、设置及内部键，即使名称含 `cache` 也不会被推荐。`tt:` 命名空间需人工判断。红色不能批量勾选；确有需要时请使用「localStorage 数据」的普通删除功能。
