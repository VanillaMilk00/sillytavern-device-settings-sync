...
- 自動上傳、自動下載分別開啟；上傳只追蹤 localStorage 原生變更並傳送差異，下載則在進入頁面時檢查。
...
localStorage 透過隱藏的同源頁框接收瀏覽器原生變更通知，不會改寫 `setItem` 等原生方法。已知生成 fetch／XHR 與原生生成事件只用來避免在請求進行中傳輸；不讀提示詞、回應、憑證或 URL 查詢值。跨來源頁框、Worker 或未知模型傳輸無法完整觀測，必要時自動上傳會暫停；自訂擴充可用以下介面回報請求生命週期：

```js
const activity = globalThis.DeviceSettingsSync.beginModelActivity();
try {
    await yourModelRequest(); // 等到串流結束或取消
} finally {
    globalThis.DeviceSettingsSync.endModelActivity(activity);
}
```

首次啟用、重新開啟頁面或變更同步範圍時會分段核對一次本機與伺服器基準；平常閒置時不讀取整份資料，也沒有定時網路請求。下載仍依選定的普通／全部／勾選範圍處理 localStorage，IndexedDB 使用者資料維持手動同步。關閉自動上傳只會清除尚未送出的自動佇列，不會關掉獨立的自動下載開關。

自動模式需要安全 HTTPS／localhost、Web Locks、Web Crypto、Web Worker、Resource Timing 及瀏覽器原生 `storage` 通知。需要同步的原瀏覽器應保持登入；其他裝置只有在執行手動同步或開啟自動下載檢查時取得新資料。伺服器及本機備份均可能含憑證，請妥善保護。

偏好、基準與排程為同步器內部資料：一般同步排除，完整本機救援備份保留，還原預設跳過。IndexedDB 自動佇列使用獨立內部資料庫，不會出現在 IndexedDB 管理器或同步範圍中。
