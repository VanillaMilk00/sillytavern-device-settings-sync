...
- 「上傳 localStorage」把目前裝置視為來源，保存 SillyTavern 帳戶／擴充設定及選定範圍的 `localStorage`。
- 「下載 localStorage」取得伺服器上對應範圍的資料，然後重新載入頁面以完整套用。
...
在「設定」啟用 IndexedDB 手動同步並選好範圍後，按鈕會改為「上傳／下載 localStorage＋已選 IndexedDB」，一併處理你選取的項目。IndexedDB 不會隨自動上傳或下載執行；設定只保存在目前網站／帳戶／裝置。
...
1. 在第一台裝置登入 SillyTavern，調整好設定後按「上傳 localStorage」。
2. 在其他裝置登入同一帳戶，按「下載 localStorage」。
3. 頁面會重新載入一次，完成套用。
