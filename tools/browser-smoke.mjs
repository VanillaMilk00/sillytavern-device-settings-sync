...
try {
    ...
    await page.evaluate(() => {
        localStorage.setItem('qa:file.json', '{"name":"中文😀"}');
        localStorage.setItem('qa:document', '[1,2]');
        localStorage.setItem('qa:api_token_cache', 'fake-credential');
        localStorage.setItem('qa:history_cache', 'important-history');
    ...
    assert.equal(await manager.locator('input[aria-label="qa:history_cache"]').isDisabled(), true);
    assert.match(await manager.locator('tr[data-node-id="key:qa:file.json"] .dss_type').innerText(), /^\.json · JSON/u);
    assert.match(await manager.locator('tr[data-node-id="key:qa:document"] .dss_type').innerText(), /\.json \(inferred\)/u);
    assert.equal(await manager.getByRole('button', { name: 'Back up and clean', exact: true }).isDisabled(), true);
    pass('three-color analysis protects credentials/history and displays explicit versus inferred extensions');
    await manager.getByRole('checkbox', { name: 'Also back up before importing, restoring or removing local data', exact: true }).uncheck();
    await manager.getByRole('button', { name: 'Select recommended items', exact: true }).click();
    assert.equal(await manager.locator('input[aria-label="tt:cache"]').isChecked(), false);
    requests.length = 0;
    await manager.getByRole('button', { name: 'Back up and clean', exact: true }).click();
    await page.locator('dialog[open]').last().locator('.popup-button-cancel').click();
    await page.waitForFunction(() => !document.querySelector('.dss_manager.dss_pending'));
    assert.equal(requests.length, 0);
    assert.equal(await page.evaluate(() => JSON.stringify(Object.entries(localStorage).sort())), beforeCleanup);
    pass('batch cleanup cancellation never changes data or creates backups');
    await page.route('**' + base + '/backups', route => route.request().method() === 'POST'
        ? route.fulfill({ status: 503, json: { code: 'backupBackendMissing' } }) : route.continue());
    await manager.getByRole('button', { name: 'Back up and clean', exact: true }).click();
    await page.locator('dialog[open]').last().locator('.popup-button-ok').click();
    ...
    assert.equal(await page.evaluate(() => JSON.stringify(Object.entries(localStorage).sort())), beforeCleanup);
    await page.unroute('**' + base + '/backups');
    pass('batch cleanup fails closed when its mandatory backup fails even with optional backups off');
    requests.length = 0;
    await manager.getByRole('button', { name: 'Back up and clean', exact: true }).click();
    await page.evaluate(() => localStorage.setItem('qa:cache', 'changed-after-preview'));
    await page.locator('dialog[open]').last().locator('.popup-button-ok').click();
    await page.waitForFunction(() => !document.querySelector('.dss_manager.dss_pending'));
    assert.equal(requests.length, 0);
    assert.equal(await page.evaluate(() => localStorage.getItem('qa:cache')), 'changed-after-preview');
    assert.match(await manager.locator('.dss_feedback').innerText(), /changed after this preview/u);
    pass('batch cleanup rechecks its preview before any backup or deletion');
    await manager.getByRole('button', { name: 'Analyze now', exact: true }).click();
    assert.equal(await manager.locator('.dss_feedback').innerText(), '');
    await manager.getByRole('button', { name: 'Select recommended items', exact: true }).click();
    await page.screenshot({ path: path.join(output, 'cleanup-desktop.png'), fullPage: true });
    requests.length = 0;
    await manager.getByRole('button', { name: 'Back up and clean', exact: true }).click();
    await page.locator('dialog[open]').last().locator('.popup-button-ok').click();
    await page.waitForFunction(() => !document.querySelector('.dss_manager.dss_pending'));
    assert.deepEqual(requests, ['POST ' + base + '/backups']);
    assert.deepEqual(await page.evaluate(() => [localStorage.getItem('qa:cache'), localStorage.getItem('tools_debug'), localStorage.getItem('qa:api_token_cache'), localStorage.getItem('qa:history_cache'), localStorage.getItem('tt:cache')]),
        [null, null, 'fake-credential', 'important-history', 'review-platform']);
    assert.match(await manager.locator('.dss_feedback').innerText(), /Cleaned 2 keys/u);
    const rescue = (await api('/backups')).body[0];
    assert.equal((await api('/backups/' + rescue.id)).body.entries.find(item => item.key === 'qa:cache').value, 'changed-after-preview');
    pass('batch cleanup saves complete rescue data, preserves protected keys and reports released capacity');
    const beforeProbe = await page.evaluate(() => JSON.stringify(Object.entries(localStorage).sort()));
    assert.match(await manager.locator('.dss_capacity_panel').innerText(), /Reference maximum: 5.00 MiB/u);
    assert.match(await manager.locator('.dss_capacity_panel').innerText(), /Actual maximum: unknown/u);
    await manager.getByRole('button', { name: 'Test capacity (optional)', exact: true }).click();
    await page.locator('dialog[open]').last().locator('.popup-button-cancel').click();
