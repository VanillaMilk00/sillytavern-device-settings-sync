...
export function planRemote(before, state, options = {}) {
    const changes = [];
    if ((options.fullStorage || options.selectedScope) && state.seeded) {
        for (const [key, value] of before) {
            const selected = !options.selectedScope || (options.selectedScope.kind === 'prefix'
                ? key.startsWith(options.selectedScope.prefix)
                : options.selectedScope.kind === 'keys' && options.selectedScope.keys.includes(key));
            if (selected && !isInternalKey(key) && !Object.hasOwn(state.entries || {}, key)) changes.push({ key, before: value, after: null });
        }
    }
