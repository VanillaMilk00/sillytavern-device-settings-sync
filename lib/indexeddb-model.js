...
export const INTERNAL_SYNC_DATABASE_PREFIX = 'device-settings-sync-internal:';
...
export async function listIndexedDatabases(factory = globalThis.indexedDB) {
    ...
    return rows.filter(row => typeof row.name === 'string' && !row.name.startsWith(INTERNAL_SYNC_DATABASE_PREFIX))
        .map(row => ({ name: row.name, version: row.version || 1 })).sort((a, b) => a.name.localeCompare(b.name));
}

function openExisting(name, factory, knownDatabases) {
    return new Promise(async (resolve, reject) => {
        let known;
        try { known = knownDatabases || await listIndexedDatabases(factory); } catch (error) { reject(error); return; }
...
function readStoreKeys(database, storeName, keyTokens) {
    return new Promise((resolve, reject) => {
        let transaction;
        try { transaction = database.transaction(storeName, 'readonly'); }
        catch (error) { reject(error); return; }
        ...
        let requests;
        try {
            requests = [...new Set(keyTokens)].map(token => {
                const key = decodeValue(JSON.parse(token));
                return { key: store.getKey(key), value: store.get(key) };
            });
        } catch (error) { reject(error); return; }
        transaction.oncomplete = () => {
            const rows = requests.filter(request => request.key.result !== undefined);
            resolve({ keys: rows.map(request => request.key.result), values: rows.map(request => request.value.result) });
        };
        transaction.onerror = () => reject(transaction.error || new IndexedDbError('indexedDbReadFailed'));
        transaction.onabort = () => reject(transaction.error || new IndexedDbError('indexedDbReadFailed'));
    });
...
export async function inspectIndexedDB({ factory = globalThis.indexedDB } = {}) {
    const rows = await listIndexedDatabases(factory);
    const result = { format: INDEXEDDB_ARCHIVE_FORMAT, version: 1, createdAt: new Date().toISOString(), scope: { kind: 'all' }, databases: [] };
    result.databases = await Promise.all(rows.map(async row => {
        const database = await openExisting(row.name, factory, rows);
        try {
            const stores = Array.from(database.objectStoreNames, name => ({ name, records: [], total: null, loaded: false, hasMore: false }));
...
export async function readIndexedDbStorePage({ databaseName, storeName, afterKeyToken, limit = 100,
    factory = globalThis.indexedDB, tolerateUnsupported = true } = {}) {
    if (typeof databaseName !== 'string' || typeof storeName !== 'string'
        || !Number.isSafeInteger(limit) || limit < 1 || limit > 500) throw new IndexedDbError('indexedDbReadFailed');
    const database = await openExisting(databaseName, factory);
    try {
        const page = await new Promise((resolve, reject) => {
            ...
            countRequest.onerror = () => reject(countRequest.error || new IndexedDbError('indexedDbReadFailed'));
            let range;
            try {
                range = afterKeyToken ? globalThis.IDBKeyRange.lowerBound(decodeIndexedDbKey(afterKeyToken), true) : undefined;
            } catch (error) { reject(new IndexedDbError('indexedDbInvalidKey', { cause: error.message })); return; }
            const cursorRequest = store.openCursor(range);
            cursorRequest.onsuccess = () => {
                const cursor = cursorRequest.result;
                if (!cursor) return;
                if (records.length >= limit) { hasMore = true; return; }
                records.push({ key: cursor.key, value: cursor.value });
                cursor.continue();
            };
            cursorRequest.onerror = () => reject(cursorRequest.error || new IndexedDbError('indexedDbReadFailed'));
            transaction.oncomplete = async () => {
                try {
                    const encoded = [];
                    for (const record of records) {
                        const key = await encodeValue(record.key);
                        let value;
                        try { value = await encodeValue(record.value); }
                        catch (error) {
                        ...
                        encoded.push({ key, keyToken: JSON.stringify(key), value });
                    }
                    resolve({ records: encoded, total, hasMore });
    ...
    for (const row of databases) {
        ...
        try {
            ...
            for (const storeName of Array.from(database.objectStoreNames)) {
                ...
                const entireStoreSelected = !selection || selection.some(item => item.keyToken === undefined);
                const rows = entireStoreSelected
                    ? await readStore(database, storeName)
                    : await readStoreKeys(database, storeName, selection.map(item => item.keyToken));
