...
export class AutoSync {
    ...
    scheduleReload(delay = 0) {
        ...
        this.reloadTimer = this.win.setTimeout(async () => {
            if (this.pendingReload === null) return;
            try {
                const reloaded = await this.withLock(() => this.reloadWhenIdle());
                if (!reloaded && this.pendingReload !== null) this.scheduleReload(2000);
            } catch (error) {
                if (error.code === 'operationBusy') this.scheduleReload(2000);
                else this.api.failure?.(error);
            }
        }, Math.max(0, delay));
    }
    fatal(error) {
    ...
    async reloadWhenIdle() {
        if (this.pendingReload === null || this.api.busy() || (await this.activity()).active) return false;
    ...
    async syncIndexedDb(direction, { before, activity, force = false } = {}) {
        let result;
        try {
            result = await this.api.syncIndexedDb?.(direction, { automaticRun: true,
                onApplied: revision => { this.pendingReload = revision ?? 0; },
                onBeforeApply: actualDirection => before && activity
                    ? this.checkLocal(before, actualDirection, activity, force || actualDirection !== direction)
                    : undefined });
        }
        catch (error) {
            if (this.pendingReload !== null) this.scheduleReload(0);
            throw error;
        }
        if (result?.direction === 'download' && result.changed && this.pendingReload === null) this.pendingReload = result.revision ?? 0;
        // IndexedDB may independently resolve a conflict in the opposite
        // direction from localStorage. Honor its actual applied direction.
        if (result?.direction === 'download' && this.pendingReload !== null && !(await this.reloadWhenIdle())) this.scheduleReload(2000);
        return result;
    }
    async tick() {
        ...
        try {
            ...
            if (this.enabled() && !this.prefs().paused && !this.state().blocked) {
                ...
                const activity = await this.activity();
                if (this.pendingReload !== null || this.entry || this.state().intent || (this.prefs().upload && this.lastEnd >= (this.prefs().armedAfter || 0)
    ...
    async perform(direction, activity) {
        ...
        if ((this.api.options().fullStorage || this.api.options().selectedScope || intent?.fullStorage || intent?.selectedScope)
            && !health.capabilities?.includes('full-storage-v1')) throw new StorageError('fullBackendMissing');
        if (intent && direction === 'upload') {
            let receipt;
            try { receipt = await this.api.request('/commits/' + intent.operationId, {}, Boolean(intent.fullStorage || intent.selectedScope)); }
            ...
            if (!this.prefs().upload) {
                // No server receipt means the uncertain upload did not commit.
                // Consume that unsent intent and let an independently enabled
                // download check run instead of pausing the whole scheduler.
                this.saveState({ intent: null });
                intent = null;
                if (!this.prefs().download) return false;
                direction = 'download';
            }
        }
        ...
        if (decision === 'equal') {
            await this.syncIndexedDb(direction, { before, activity, force });
            await this.remember(local, remote);
            return true;
        }
        ...
        if (direction === 'upload') {
            if (!intent) {
                intent = { operationId: uid(), expectedRevision: remote.revision, localHash, remoteHash,
                    filter: JSON.stringify(options), fullStorage: options.fullStorage === true,
                    ...(options.selectedScope ? { selectedScope: options.selectedScope } : {}), cycle: activity.cycle };
                // A small durable journal, never a duplicate of credential values.
                this.saveState({ intent });
            }
            const body = options.fullStorage || options.selectedScope ? {
                operationId: intent.operationId, expectedRevision: intent.expectedRevision,
                deviceId: this.api.persistDevice(), scope: options.selectedScope || { kind: 'full' },
                entries: sortedEntries(local).map(([key, value]) => ({ key, value })),
            } : {
                operationId: intent.operationId, expectedRevision: intent.expectedRevision,
                deviceId: this.api.persistDevice(), mutations: changes,
            };
            const receipt = await this.api.request('/commit', { method: 'POST', body: JSON.stringify(body) }, Boolean(options.fullStorage || options.selectedScope));
            ...
            this.pendingReload = remote.revision ?? 0;
            try {
            ...
            if (this.pendingReload !== null && !(await this.reloadWhenIdle())) this.scheduleReload(2000);
        }
        if (direction === 'upload') await this.syncIndexedDb(direction, { before, activity, force });
        this.rescue = null;
        this.api.changed();
        return true;
    }
