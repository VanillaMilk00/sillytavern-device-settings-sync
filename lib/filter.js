...
function selectedScopeContains(key, scope) {
    if (!scope || typeof scope !== 'object') return false;
    if (scope.kind === 'full') return true;
    if (scope.kind === 'prefix') return typeof scope.prefix === 'string' && key.startsWith(scope.prefix);
    return scope.kind === 'keys' && Array.isArray(scope.keys) && scope.keys.includes(key);
}
...
export function classifyStorageEntry(key, value, options = {}) {
    ...
    if (options.selectedScope) {
        return selectedScopeContains(normalizedKey, options.selectedScope)
            ? { portable: true, reason: 'selected-sync' }
            : { portable: false, reason: 'not-selected' };
    }
